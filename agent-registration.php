...
if(isset($_POST['form-submit'])){
    try{
        if($_POST['full_name'] == '') {throw new Exception("Full Name can not be empty");}
        ...
        if($_POST['password'] == ''||['retype_password'] == '') {throw new Exception("Password can not be empty");}
        if($_POST['password'] != $_POST['retype_password']) {throw new Exception("Password and Confirm Password does not match");}


        $statement = $pdo->prepare("SELECT * FROM agents WHERE email=?");
        $statement->execute([$_POST['email']]);
        $total = $statement->rowCount();
        if($total) {throw new Exception("Email already exists.");}

        $password = password_hash($_POST['password'], PASSWORD_DEFAULT);
        $token=time();
        $statement = $pdo->prepare("INSERT INTO agents (full_name,email,photo,password,token,status) VALUES (?,?,?,?,?,?)");
        ...
        try {
            ...
            $mail->isHTML(true);
            $mail->Subject = 'Agent Registration Verification Email';
        ...
        $success_message = "Registration is successful, check your email and verify your account.";
                ...
                <div class="col-xl-4 col-lg-5 col-md-6 col-sm-12">
                    <div class="login-form">
                        <form action="" method="post">
                            ...
                            <div class="mb-3">
                                <button type="submit" class="btn btn-primary bg-website" name="form-submit">
                                    Create Account
                                </button>
                            </div>
                        </form>
